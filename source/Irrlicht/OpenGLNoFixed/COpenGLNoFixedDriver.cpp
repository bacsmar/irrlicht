...
#include "COGLFixedPipelineRenderer.h"
...
COpenGLNoFixedDriver::COpenGLNoFixedDriver(const SIrrlichtCreationParameters& params, io::IFileSystem* io, IContextManager* contextManager) :
	CNullDriver(io, params.WindowSize), COpenGLNoFixedExtensionHandler(), CacheHandler(0), MaterialRenderer2D(0), CurrentRenderMode(ERM_NONE),
...
{
#ifdef _DEBUG
	setDebugName("COpenGLNoFixedDriver");
#endif
...
COpenGLNoFixedDriver::~COpenGLNoFixedDriver()
{
	...
	bool COpenGLNoFixedDriver::genericDriverInit(const core::dimension2d<u32>& screenSize, bool stencilBuffer)
	{
		...
		// reset cache handler
		delete CacheHandler;
		CacheHandler = new COpenGLNoFixedCacheHandler(this);
...
//		DriverAttributes->setAttribute("MaxMultipleRenderTargets", MaxMultipleRenderTargets);
	...
	void COpenGLNoFixedDriver::loadShaderData(const io::path& vertexShaderName, const io::path& fragmentShaderName, c8** vertexShaderData, c8** fragmentShaderData)
	{
	...
	void COpenGLNoFixedDriver::createMaterialRenderers()
	{
		// Create callbacks.
		...
		MaterialRenderer2D = new COGLNoFixedRenderer2D(vs2DData, fs2DData, this);
		delete[] vs2DData;
		delete[] fs2DData;
	}

	bool COpenGLNoFixedDriver::beginScene(u16 clearFlag, SColor clearColor, f32 clearDepth, u8 clearStencil, const SExposedVideoData& videoData, core::rect<s32>* sourceRect)
	{
	...
	bool COpenGLNoFixedDriver::endScene()
	{
	...
	const core::matrix4& COpenGLNoFixedDriver::getTransform(E_TRANSFORMATION_STATE state) const
	{
	...
	//! sets transformation
	void COpenGLNoFixedDriver::setTransform(E_TRANSFORMATION_STATE state, const core::matrix4& mat)
	{
		Matrices[state] = mat;
	...
	bool COpenGLNoFixedDriver::updateVertexHardwareBuffer(SHWBufferLink_opengl *HWBuffer)
	{
		if (!HWBuffer)
			return false;
	...
	bool COpenGLNoFixedDriver::updateIndexHardwareBuffer(SHWBufferLink_opengl *HWBuffer)
	{
		if (!HWBuffer)
			return false;
	...
	bool COpenGLNoFixedDriver::updateHardwareBuffer(SHWBufferLink *HWBuffer)
	{
		if (!HWBuffer)
			return false;
	...
	COpenGLNoFixedDriver::SHWBufferLink *COpenGLNoFixedDriver::createHardwareBuffer(const scene::IMeshBuffer* mb)
	{
	...
	void COpenGLNoFixedDriver::deleteHardwareBuffer(SHWBufferLink *_HWBuffer)
	{
		if (!_HWBuffer)
			return;
	...
	void COpenGLNoFixedDriver::drawHardwareBuffer(SHWBufferLink *_HWBuffer)
	{
		if (!_HWBuffer)
			return;
	...
	IRenderTarget* COpenGLNoFixedDriver::addRenderTarget()
	{
	...
	void COpenGLNoFixedDriver::drawVertexPrimitiveList(const void* vertices, u32 vertexCount,
			...
			E_VERTEX_TYPE vType, scene::E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType)
	{
		...
		case EVT_2TCOORDS:
			...
			if (vertices)
			{
				...
				extGlVertexAttribPointer(EVA_COLOR, 4, GL_UNSIGNED_BYTE, true, sizeof(S3DVertex2TCoords), &(static_cast<const S3DVertex2TCoords*>(vertices))[0].Color);
		...
		case EVT_TANGENTS:
			...
			if (vertices)
			{
				...
				extGlVertexAttribPointer(EVA_COLOR, 4, GL_UNSIGNED_BYTE, true, sizeof(S3DVertexTangents), &(static_cast<const S3DVertexTangents*>(vertices))[0].Color);
...
#define GL_UNSIGNED_INT 0x1405
	...
	void COpenGLNoFixedDriver::draw2DImage(const video::ITexture* texture, const core::position2d<s32>& destPos,
	...
	void COpenGLNoFixedDriver::draw2DImage(const video::ITexture* texture, const core::rect<s32>& destRect,
	...
	void COpenGLNoFixedDriver::draw2DImage(const video::ITexture* texture, u32 layer, bool flip)
	{
		chooseMaterial2D();
		Material.TextureLayer[0].Texture = const_cast<ITexture*>(texture);
		...
		extGlEnableVertexAttribArray(EVA_COLOR);
		extGlEnableVertexAttribArray(EVA_TCOORD0);
		extGlVertexAttribPointer(EVA_POSITION, 3, GL_FLOAT, false, sizeof(S3DVertex), &(static_cast<const S3DVertex*>(quad2DVertices))[0].Pos);
		extGlVertexAttribPointer(EVA_COLOR, 4, GL_UNSIGNED_BYTE, true, sizeof(S3DVertex), &(static_cast<const S3DVertex*>(quad2DVertices))[0].Color);
		extGlVertexAttribPointer(EVA_TCOORD0, 2, GL_FLOAT, false, sizeof(S3DVertex), &(static_cast<const S3DVertex*>(quad2DVertices))[0].TCoords);
	...
	void COpenGLNoFixedDriver::draw2DImageBatch(const video::ITexture* texture,
			...
			SColor color, bool useAlphaChannelOfTexture)
	{
		...
		if (vtx.size())
		{
			...
			extGlEnableVertexAttribArray(EVA_COLOR);
			extGlEnableVertexAttribArray(EVA_TCOORD0);
			extGlVertexAttribPointer(EVA_POSITION, 3, GL_FLOAT, false, sizeof(S3DVertex), &vtx[0].Pos);
			extGlVertexAttribPointer(EVA_COLOR, 4, GL_UNSIGNED_BYTE, true, sizeof(S3DVertex), &vtx[0].Color);
			extGlVertexAttribPointer(EVA_TCOORD0, 2, GL_FLOAT, false, sizeof(S3DVertex), &vtx[0].TCoords);
			...
			bool useAlphaChannelOfTexture)
	{
		...
		if (vertices.size())
		{
			...
			extGlEnableVertexAttribArray(EVA_COLOR);
			extGlEnableVertexAttribArray(EVA_TCOORD0);
			extGlVertexAttribPointer(EVA_POSITION, 3, GL_FLOAT, false, sizeof(S3DVertex), &vertices[0].Pos);
			extGlVertexAttribPointer(EVA_COLOR, 4, GL_UNSIGNED_BYTE, true, sizeof(S3DVertex), &vertices[0].Color);
			extGlVertexAttribPointer(EVA_TCOORD0, 2, GL_FLOAT, false, sizeof(S3DVertex), &vertices[0].TCoords);
	...
	//! draw a 2d rectangle
	void COpenGLNoFixedDriver::draw2DRectangle(SColor color,
	...
	void COpenGLNoFixedDriver::draw2DRectangle(const core::rect<s32>& position,
	...
	//! Draws a 2d line.
	void COpenGLNoFixedDriver::draw2DLine(const core::position2d<s32>& start,
	...
	//! Draws a pixel
	void COpenGLNoFixedDriver::drawPixel(u32 x, u32 y, const SColor &color)
	{
		const core::dimension2d<u32>& renderTargetSize = getCurrentRenderTargetSize();
	...
	ITexture* COpenGLNoFixedDriver::createDeviceDependentTexture(const io::path& name, IImage* image)
	{
	...
	ITexture* COpenGLNoFixedDriver::createDeviceDependentTextureCubemap(const io::path& name, const core::array<IImage*>& image)
	{
	...
	//! Sets a material.
	void COpenGLNoFixedDriver::setMaterial(const SMaterial& material)
	{
		Material = material;
	...
	void COpenGLNoFixedDriver::setRenderStates3DMode()
	{
	...
	void COpenGLNoFixedDriver::setBasicRenderStates(const SMaterial& material, const SMaterial& lastmaterial, bool resetAllRenderStates)
	{
	...
	void COpenGLNoFixedDriver::setTextureRenderStates(const SMaterial& material, bool resetAllRenderstates)
	{
	...
	GLint COpenGLNoFixedDriver::getTextureWrapMode(u8 clamp) const
	{
		switch (clamp)
		{
			case ETC_CLAMP:
	...
	void COpenGLNoFixedDriver::setRenderStates2DMode(bool alpha, bool texture, bool alphaChannel)
	{
		...
		Material.setTexture(0, const_cast<COpenGLTexture*>(CacheHandler->getTextureCache().get(0)));
	...
	void COpenGLNoFixedDriver::chooseMaterial2D()
	{
		if (!OverrideMaterial2DEnabled)
	...
	const wchar_t* COpenGLNoFixedDriver::getName() const
	{
		return Name.c_str();
	}
	...
	void COpenGLNoFixedDriver::deleteAllDynamicLights()
	{
		RequestedLights.clear();
	...
	s32 COpenGLNoFixedDriver::addDynamicLight(const SLight& light)
	{
	...
	void COpenGLNoFixedDriver::turnLightOn(s32 lightIndex, bool turnOn)
	{
	...
	u32 COpenGLNoFixedDriver::getMaximalDynamicLightAmount() const
	{
		return 8;
	}

	void COpenGLNoFixedDriver::setViewPort(const core::rect<s32>& area)
	{
	...
	void COpenGLNoFixedDriver::drawStencilShadowVolume(const core::array<core::vector3df>& triangles, bool zfail, u32 debugDataVisible)
	{
		...
		extGlEnableVertexAttribArray(EVA_POSITION);
		extGlVertexAttribPointer(EVA_POSITION, 3, GL_FLOAT, false, sizeof(core::vector3df), triangles.const_pointer());
	...
	void COpenGLNoFixedDriver::drawStencilShadow(bool clearStencilBuffer,
	...
	//! Draws a 3d line.
	void COpenGLNoFixedDriver::draw3DLine(const core::vector3df& start,
	...
	void COpenGLNoFixedDriver::OnResize(const core::dimension2d<u32>& size)
	{
	...
	ECOLOR_FORMAT COpenGLNoFixedDriver::getColorFormat() const
	{
		return ColorFormat;
	}
	...
	s32 COpenGLNoFixedDriver::getVertexShaderConstantID(const c8* name)
	{
		return getPixelShaderConstantID(name);
	}

	//! Get a pixel shader constant index.
	s32 COpenGLNoFixedDriver::getPixelShaderConstantID(const c8* name)
	{
	...
	void COpenGLNoFixedDriver::setVertexShaderConstant(const f32* data, s32 startRegister, s32 constantAmount)
	{
		os::Printer::log("Error: Please call services->setVertexShaderConstant(), not VideoDriver->setPixelShaderConstant().");
	}

	//! Sets a pixel shader constant.
	void COpenGLNoFixedDriver::setPixelShaderConstant(const f32* data, s32 startRegister, s32 constantAmount)
	{
		os::Printer::log("Error: Please call services->setPixelShaderConstant(), not VideoDriver->setPixelShaderConstant().");
	}

	//! Sets a constant for the vertex shader based on an index.
	bool COpenGLNoFixedDriver::setVertexShaderConstant(s32 index, const f32* floats, int count)
	{
	...
	bool COpenGLNoFixedDriver::setVertexShaderConstant(s32 index, const s32* ints, int count)
	{
		return setPixelShaderConstant(index, ints, count);
	}

	//! Sets a constant for the pixel shader based on an index.
	bool COpenGLNoFixedDriver::setPixelShaderConstant(s32 index, const f32* floats, int count)
	{
		os::Printer::log("Error: Please call services->setPixelShaderConstant(), not VideoDriver->setPixelShaderConstant().");
		return false;
	}
	...
	//! Int interface for the above.
	bool COpenGLNoFixedDriver::setPixelShaderConstant(s32 index, const s32* ints, int count)
	{
		os::Printer::log("Error: Please call services->setPixelShaderConstant(), not VideoDriver->setPixelShaderConstant().");
		return false;
	}
	...
	s32 COpenGLNoFixedDriver::addShaderMaterial(const c8* vertexShaderProgram,
	...
	s32 COpenGLNoFixedDriver::addHighLevelShaderMaterial(
	...
	{
		s32 nr = -1;
		COGLNoFixedMaterialRenderer* r = new COGLNoFixedMaterialRenderer(
	...
	IVideoDriver* COpenGLNoFixedDriver::getVideoDriver()
	{
		return this;
	}
	...
	IGPUProgrammingServices* COpenGLNoFixedDriver::getGPUProgrammingServices()
	{
		return this;
	}

	ITexture* COpenGLNoFixedDriver::addRenderTargetTexture(const core::dimension2d<u32>& size,
	...
	u32 COpenGLNoFixedDriver::getMaximalPrimitiveCount() const
	{
		return 65535;
	}

	bool COpenGLNoFixedDriver::setRenderTargetEx(IRenderTarget* target, u16 clearFlag, SColor clearColor, f32 clearDepth, u8 clearStencil)
	{
	...
	void COpenGLNoFixedDriver::clearBuffers(u16 flag, SColor color, f32 depth, u8 stencil)
	{
		GLbitfield mask = 0;
	...
	IImage* COpenGLNoFixedDriver::createScreenShot(video::ECOLOR_FORMAT format, video::E_RENDER_TARGET target)
	{
...
//			glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_TYPE, &type);
	...
	void COpenGLNoFixedDriver::removeTexture(ITexture* texture)
	{
		if (!texture)
			return;
	...
	bool COpenGLNoFixedDriver::setClipPlane(u32 index, const core::plane3df& plane, bool enable)
	{
	...
	void COpenGLNoFixedDriver::enableClipPlane(u32 index, bool enable)
	{
		UserClipPlane[index].Enabled = enable;
	}

	//! Get the ClipPlane Count
	u32 COpenGLNoFixedDriver::getClipPlaneCount() const
	{
		return UserClipPlane.size();
	}

	const core::plane3df& COpenGLNoFixedDriver::getClipPlane(irr::u32 index) const
	{
		if (index < UserClipPlane.size())
	...
	core::dimension2du COpenGLNoFixedDriver::getMaxTextureSize() const
	{
		return core::dimension2du(MaxTextureSize, MaxTextureSize);
	}

	GLenum COpenGLNoFixedDriver::getGLBlend(E_BLEND_FACTOR factor) const
	{
	...
	GLenum COpenGLNoFixedDriver::getZBufferBits() const
	{
		GLenum bits = 0;
	...
	COpenGLNoFixedCacheHandler* COpenGLNoFixedDriver::getCacheHandler() const
	{
		return CacheHandler;
	}
...
#endif // _IRR_COMPILE_WITH_OGL_NOFIXED_
...
#ifndef _IRR_COMPILE_WITH_OGL_NOFIXED_
class IVideoDriver;
...
{
#ifdef _IRR_COMPILE_WITH_OGL_NOFIXED_
	return new COpenGLNoFixedDriver(params, io, contextManager);
#else
	return 0;
#endif //  _IRR_COMPILE_WITH_OGL_NOFIXED_
}
